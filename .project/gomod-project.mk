...
vars:
	...
	echo "PROJ_DATA_DIR=$(PROJ_DATA_DIR)"
...
fmt-check:
	echo "Running Fmt check in $(shell pwd)"
	@test -z "$(shell gofmt -l -r '"github.com/pkg/errors" -> "github.com/cockroachdb/errors"' ./ | tee /dev/stderr)"
	@test -z "$(shell gofmt -l -r 'interface{} -> any' -r 'github.com/alecthomas/assert -> github.com/stretchr/testify/assert' -r 'fmt.Errorf -> errors.Errorf' ./ | tee /dev/stderr)"
	echo "Done fmt check"
...
lint-custom:
	...
	custom-linters ./internal/llm/...
	echo "Done custom linters"

# staticcheck and unused are memory-heavy and may cause OOM on ubuntu-latest-m CI runners.
# They are enabled by default for local runs.
# GitHub Actions sets CI=true automatically; to skip them locally: CI=true make lint
ifeq ($(CI),true)
GOLANGCI_DISABLE_FLAGS := --disable=staticcheck --disable=unused
else
GOLANGCI_DISABLE_FLAGS :=
endif

lint: fmt # lint-custom
	echo "Running lint"
	golangci-lint run -v --timeout=20m $(GOLANGCI_DISABLE_FLAGS) ./...
	echo "Done lint"
