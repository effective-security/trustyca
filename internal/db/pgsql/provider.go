...
import (
	...
	"github.com/effective-security/xdb/pkg/notifier"
	"github.com/effective-security/xdb/xsql"
	"github.com/effective-security/xlog"
)

var logger = xlog.NewPackageLogger("github.com/effective-security/trustyca/internal/db", "pgsql")
...
func (p *Provider) ConnectionString() string {
	return p.prov.ConnectionString()
}
...
// If the context is cancelled, the sql package will roll back
// the transaction. Tx.Commit will return an error if the context provided to
// BeginTx is cancelled.
//
...
func DbMeasureSince(start time.Time) {
	...
	DbMeasureQuerySince(method, start)
}
...
func DbMeasureQuerySince(query string, start time.Time) {
	...
	if elapsedMS > int64(DbSlowMethodMilliseconds) {
		logger.KV(xlog.DEBUG, "reason", "slow", "db", "trustycadb", "query", query, "ms", elapsedMS)
	}
	metricskey.PerfDbOperation.MeasureSince(start, query)

	if idx := strings.Index(query, "_"); idx >= 0 {
		query = query[:idx]
	}
	// Count the number of queries, without the suffix
	metricskey.StatsDbQueryCount.IncrCounter(1, query)
}
...
func (p *Provider) NewNotificationsListener() notifier.Listener {
	return notifier.NewListener(p, 10*time.Millisecond, time.Minute)
}
